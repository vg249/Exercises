...
def run_threaded(num_threads, N):
    ...
    # Decalring mp object to send as argument for thread worker
    mp = MP()
    # Variable to sum up the results from the threads
    total_result = 0
    thread_worker = [ThreadedWorker(mp,i,num_threads,N) for i in range(num_threads)]
    
    for j in range(num_threads): 
        thread_worker[j].start()
    ...
    for k in range(num_threads):    
        total_result += thread_worker[k].result
...
def run_parent(num_children, N):
    ...
    child = [Popen([sys.executable, sys.argv[0], sys.argv[1], 'child', str(i), str(num_children)]) for i in range(num_children)]
    result_total = 0

    # terminating the child process to receive the results
    for j in range(num_children):
        result_total += child[j].wait()
...
def run_child(N, args):
    ...
    # Running the steps based on number of child process created
