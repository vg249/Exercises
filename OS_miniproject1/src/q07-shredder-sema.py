...
class ShredderScheduler(MP):
    ...
    def __init__(self, num_jobs):
        MP.__init__(self)
        # TODO
        # Sema for Shredders, initializing it to maximum threaders available
        self.semaShredder = self.Semaphore("semaShred",0)
    
        # Sema for Paper, Initializing it to 1
        self.semaPaper    = self.Semaphore("semaPaper",0)
        
        # Seperate semaphores to give atomicity to countPaper
        self.semaMutex     = self.Semaphore("countPaper",1)
        self.countPap        = 0
    ...
    def shredder_ready(self):
        ...
        # TODO
        self.semaShredder.procure()
        self.semaPaper.vacate()
    ...
    def add_paper(self):
        ...
        # TODO
        self.semaMutex.procure()
        self.countPap += 1
        if self.countPap <= self.numJobs:
            self.semaMutex.vacate()
            self.semaShredder.vacate()
            self.semaPaper.procure()
            self.semaMutex.procure()
            self.countPap -= 1
            self.semaMutex.vacate()
        else:
